...
function CardSkeleton() {
  return (
    <div className="rounded-2xl overflow-hidden border border-amber-200/70 bg-white">
      <div className="aspect-[16/10] bg-gradient-to-br from-amber-100 to-orange-100 animate-pulse" />
      <div className="p-5 space-y-3">
        <div className="h-4 w-3/4 rounded bg-amber-100 animate-pulse" />
        <div className="h-3 w-full rounded bg-amber-50 animate-pulse" />
        <div className="h-3 w-5/6 rounded bg-amber-50 animate-pulse" />
        <div className="h-9 w-full rounded-lg bg-amber-100 animate-pulse mt-4" />
      </div>
    </div>
  );
}
...
export default function EventsPage() {
  ...
  return (
    <Layout>
          ...
          >
            ...
            {!loading && (
              <div className="mt-7 flex flex-wrap items-center gap-2.5">
                <span className="inline-flex items-center gap-1.5 px-3 h-8 rounded-lg bg-white/70 backdrop-blur border border-amber-200 text-stone-700 text-[12.5px] font-medium">
                  <span className="size-1.5 rounded-full bg-amber-500" />
                  {counts.UPCOMING} upcoming
                </span>
                {counts.ONGOING > 0 && (
                  <span className="inline-flex items-center gap-1.5 px-3 h-8 rounded-lg bg-white/70 backdrop-blur border border-emerald-200 text-emerald-800 text-[12.5px] font-medium">
                    <span className="size-1.5 rounded-full bg-emerald-500 animate-pulse" />
                    {counts.ONGOING} live now
                  </span>
                )}
                <span className="inline-flex items-center gap-1.5 px-3 h-8 rounded-lg bg-white/70 backdrop-blur border border-stone-200 text-stone-600 text-[12.5px] font-medium">
      ...
      <section className="bg-white/85 backdrop-blur-md border-b border-amber-200/70 sticky top-under-header z-40">
        <div className="container mx-auto px-4 py-3 sm:py-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            {/* Segmented filter pills */}
            <div className="no-scrollbar -mx-1 flex flex-nowrap items-center gap-1 overflow-x-auto bg-amber-50 rounded-full p-1 ring-1 ring-amber-200/70 sm:mx-0">
              {TAB_DEFS.map((tab) => {
                  ...
                  >
                    {isActive && (
                      <motion.span
                        ...
                        className="absolute inset-0 rounded-full bg-white shadow-sm ring-1 ring-amber-300/60"
                      />
                    )}
            ...
            <div className="relative sm:ml-auto sm:max-w-[300px] w-full">
              ...
              <input
                ...
                className="w-full h-9 pl-9 pr-3 rounded-full bg-amber-50/60 border border-amber-200/70 text-[13px] text-stone-800 placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-amber-400/40 focus:border-amber-400"
              />
            </div>
          </div>
        </div>
      ...
      {/* Body */}
      <section className="py-10 sm:py-14 bg-gradient-to-b from-amber-50/40 to-white min-h-[60vh]">
                ...
                {filteredEvents
                  ...
                  return (
                    <motion.article
                      ...
                      className="group relative flex flex-col rounded-2xl overflow-hidden bg-white border border-amber-200/60 hover:border-amber-300 shadow-[0_1px_3px_rgba(120,53,15,0.04)] hover:shadow-[0_18px_36px_-18px_rgba(120,53,15,0.30)] transition-all duration-300 hover:-translate-y-0.5"
                    >
                      <Link to={eventHref} className="block relative">
                        <div className="relative overflow-hidden bg-gradient-to-br from-amber-200 to-orange-200 aspect-[16/10]">
                          ...
                          {/* Date badge — top-left */}
                          {day !== null && (
                            <div className="absolute top-3 left-3 flex flex-col items-center justify-center w-12 h-12 rounded-xl bg-white/95 backdrop-blur shadow-md ring-1 ring-black/5">
                              <span className="text-[9px] font-bold uppercase tracking-wider text-amber-700">{monthShort}</span>
                              <span className="text-[16px] font-bold leading-none text-stone-900">{day}</span>
                            </div>
                          )}
                          ...
                          <div className="absolute top-3 right-3 flex flex-col items-end gap-1.5">
                            ...
                            {event.featured && (
                              <span className="inline-flex items-center gap-1 px-2 h-6 rounded-full bg-white/95 text-amber-700 text-[10.5px] font-semibold shadow-sm">
                      ...
                      <div className="flex flex-col flex-1 p-5">
                        <Link to={eventHref} className="block">
                          <h3 className="text-[17px] font-semibold text-stone-900 leading-snug tracking-tight group-hover:text-amber-700 transition-colors line-clamp-2">
                            {event.title}
                          </h3>
                        </Link>
                        {(event.shortDescription || event.description) && (
                          <p className="mt-1.5 text-[13px] text-stone-600 leading-relaxed line-clamp-2">
                            {event.shortDescription || event.description}
                          </p>
                        )}

                        {/* Meta */}
                        <dl className="mt-4 space-y-1.5 text-[12.5px] text-stone-600">
...
function SpotlightCard({
  ...
  return (
    <article className="relative rounded-3xl overflow-hidden border border-amber-200/70 bg-white shadow-[0_24px_60px_-30px_rgba(120,53,15,0.35)]">
      <div className="grid lg:grid-cols-5 gap-0">
        {/* Image */}
        <Link to={eventHref} className="relative lg:col-span-3 block aspect-[16/10] lg:aspect-auto lg:min-h-[340px] bg-gradient-to-br from-amber-200 to-orange-300 overflow-hidden">
          {event.imageUrl ? (
            <img
              src={processImageUrl(event.imageUrl, 'card')}
              alt={event.title}
          ...
          {day !== null && (
            <div className="absolute bottom-4 left-4 flex flex-col items-center justify-center w-16 h-16 rounded-2xl bg-white/95 backdrop-blur shadow-lg ring-1 ring-black/5">
              <span className="text-[10px] font-bold uppercase tracking-wider text-amber-700">{monthShort}</span>
              <span className="text-[22px] font-bold leading-none text-stone-900">{day}</span>
            </div>
          )}
        </Link>
        ...
        <div className="lg:col-span-2 p-6 sm:p-8 flex flex-col">
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            ...
            {event.eventType && (
              <span className="inline-flex items-center px-2 h-6 rounded-md bg-amber-50 border border-amber-200 text-amber-800 text-[10.5px] font-medium">
                {event.eventType}
              </span>
            )}
          </div>
          <Link to={eventHref}>
            <h2 className="text-2xl sm:text-3xl font-bold text-stone-900 tracking-tight leading-tight hover:text-amber-700 transition-colors">
              {event.title}
            </h2>
          </Link>
          {(event.shortDescription || event.description) && (
            <p className="mt-3 text-[14px] text-stone-600 leading-relaxed line-clamp-3">
              {event.shortDescription || event.description}
            </p>
          )}

          <dl className="mt-5 space-y-2 text-[13px] text-stone-700">
            <div className="flex items-center gap-2">
