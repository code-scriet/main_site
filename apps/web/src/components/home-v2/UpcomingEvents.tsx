...
export function UpcomingEvents() {
  ...
  const { token } = useAuth();
  // Cached via React Query (keyed by token) so it isn't re-hit on every mount /
  // theme toggle — one fetch per user, shared with anything else on this key.
  const { data: myRegistrations } = useQuery({
    queryKey: ['home-my-registrations', token],
    queryFn: () => api.getMyRegistrations(token as string),
    enabled: Boolean(token),
    staleTime: 5 * 60 * 1000,
  });
  const visibleRegisteredEventIds = useMemo(
    () => (token && myRegistrations ? new Set(myRegistrations.map((r) => r.eventId)) : EMPTY_REGISTERED_IDS),
