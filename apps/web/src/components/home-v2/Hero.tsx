...
export function Hero() {
  ...
  const { isMobile, shouldReduceMotion, disableParallax } = useMotionConfig();
  const { scrollY } = useScroll();

  // Fade (+ subtle scale/parallax) the hero out as the page scrolls — mirrors
  // the light homepage's Hero so the top fades away on scroll in both themes.
            ...
            <motion.div variants={item} className="mb-7 flex flex-col items-center gap-4 lg:items-start">
              <span className="home-logo-wrap">
                ...
                <span className="relative grid h-28 w-28 place-items-center overflow-hidden rounded-3xl border border-white/15 bg-white/10 shadow-2xl backdrop-blur-md">
                  <img src="/logo.jpeg" alt="code.scriet" width={96} height={96} decoding="async" className="h-24 w-24 rounded-2xl object-cover" />
                </span>
              </span>
