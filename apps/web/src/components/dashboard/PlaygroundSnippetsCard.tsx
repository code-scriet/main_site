...
export function PlaygroundSnippetsCard() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const [snippetData, statsData, historyData] = await Promise.all([
        api.getPlaygroundSnippets(token),
        api.getPlaygroundStats(token),
        api.getPlaygroundHistory(token),
      ]);

      setSnippets(snippetData.slice(0, 5));
      setStats(statsData.languageStats || []);
      setTotalExecutions(statsData.totalExecutions || 0);
      setTodayCount(statsData.todayCount || 0);
      setDailyLimit(statsData.dailyLimit || 100);
      setHistory(historyData.slice(0, 10));
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load playground activity');
    } finally {
      setLoading(false);
    }
  }, [token]);
