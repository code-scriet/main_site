...
export function PollCard({
  ...
  return (
    <Card className={cn('border-gray-200 shadow-sm hover:shadow-md', className)}>
      <CardHeader className="space-y-3 border-b border-gray-100 pb-4">
        ...
        <div className="space-y-1.5">
          <CardTitle className="text-xl leading-snug text-gray-950 sm:text-2xl">{poll.question}</CardTitle>
          {poll.description && (
      ...
      <CardContent className="space-y-5 pt-5">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <Link to={`/polls/${poll.slug}`} className="shrink-0">
            <Button variant={compact ? 'outline' : 'default'} size="sm">
              {actionLabel}
            </Button>
          </Link>
        </div>
