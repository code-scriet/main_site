...
├── robots.txt               # SEO crawlers config (points to /sitemap.xml)
└── _redirects               # Vercel/Netlify redirects
```
**Note**: The logo file is tracked in git and should NOT be ignored. The sitemap is generated by the API and proxied through the frontend domain via [apps/web/vercel.json](apps/web/vercel.json).
