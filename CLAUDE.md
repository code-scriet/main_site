...
3. **Prisma pool size is owner-tuned via the `connection_limit` query param on `DATABASE_URL`** (Render dashboard env, **NOT code** — `resolvePoolMax()` in [apps/api/src/lib/prisma.ts](apps/api/src/lib/prisma.ts) derives `max` from the URL, code fallback **5**). Owner-approved target (July 2026): **`connection_limit=20`** — sized from a stress proof that raising the pool 5→20 drained a 40-request burst **5.4× faster**; the Neon free-tier pooler budget (~64 server conns) is shared with the playground's hardcoded `max:5`. Still forbidden without approval: adding pool middleware, touching the `datasource` block, or raising the pool **beyond 20**.
...
| Auth | Passport (Google + GitHub OAuth) + JWT (7-day) + bcrypt (native, **6.x** — swapped from bcryptjs, then 5→6 to drop the transitive `tar`/`node-pre-gyp` advisories; `$2b$` hashes verify unchanged across all versions) |
| Real-time | Socket.io: `/quiz`, `/attendance`, `/notifications` on the **main API**; `/competition` on the **playground execute-server** (relay) |
...
| Optional native | `better-sqlite3` (S6 quiz crash snapshots) + `eiows` (C++ WS engine, **default**, api + playground) — both `optionalDependencies` with try/require fallbacks: a failed native build degrades to a logged no-op / stock `ws`, never a broken install or boot (see `WS_ENGINE`/`WS_ENGINE_STRICT`) |
  ...
  api/                Express backend (TS, ESM) — entry src/index.ts
    src/
      ...
      utils/                        attendanceToken, audit, email, eventStatus,
                                    ...
                                    judgeFrames (frame protocol + nonce/tamper checks)
      utils/judgeHarnesses/         Per-language judge wrappers (python/java/javascript/cpp).
                                    Python + C++ fork per test; each reads the run nonce
                                    from stdin and emits `__JUDGE_<nonce>:` frames.
...
2. HTTP + Socket.io (`/quiz` + `/attendance` + `/notifications`). The `/competition` namespace runs on the playground execute-server (relay); the main API pushes to it over internal HTTP.
...
- **Rate limits (NAT-safe keying, contest capacity):** the general + auth limiters key by **verified user, not IP** — [apps/api/src/utils/rateLimitKey.ts](apps/api/src/utils/rateLimitKey.ts) (unit-tested) grants `u:<userId>` buckets ONLY after sync HMAC `verifyToken` (garbage/forged tokens stay pinned to the IP bucket, so bucket-minting by token rotation is impossible; the 5-min IP fallback still resolves via `getClientIp`). General: **800/15min per user** (sized for the contest arena's REST-polling fallback) · **2000/15min per anonymous IP** (shared only by unauthenticated traffic now). Auth: per-user 300 (covers `/auth/me`) · anonymous **per-(IP,email) 15 failed tries** (a 200-student NAT login storm works; per-account brute force from one IP is TIGHTER than the old shared 50) · plain-IP (OAuth redirects/exchange) 50 — all `skipSuccessfulRequests`. Others unchanged: password reset 20/15min, hiring apply 15/15min/IP, polls vote/feedback 60/15min/IP (deliberately above the teams-join 15 — a hall voting behind one campus NAT must survive), teams join 15/15min, mail `emails[]` capped at 500/request. **Socket connect limiter (NAT-safe too):** the Socket.io connection limiter in [apps/api/src/utils/socket.ts](apps/api/src/utils/socket.ts) (root + `/notifications`) keys on `resolveSocketConnectKey` ([apps/api/src/utils/socketConnectKey.ts](apps/api/src/utils/socketConnectKey.ts), unit-tested) — a handshake with a VALID access token (sync `verifyToken`; token from `handshake.auth.token`, then Bearer header, then `scriet_session` cookie) gets a per-user `u:<userId>` bucket at **30/user/60s**; invalid/absent/forged/purpose-carrying tokens fall to the `ip:<ip>` bucket at 30/IP/60s (abuse-only; anonymous sockets are rejected by namespace auth anyway). Previously the whole campus NAT shared one 30/60s bucket → only 30 dashboard sockets/min campus-wide during a contest. Map bounded (one entry per active key, swept at 2×window). `/quiz` + `/attendance` never used this limiter; `/competition` is on the playground relay — untouched.
...
| `/api/events/*` | Mixed | `POST /:id/message-registrants` (creator or ADMIN/PRESIDENT) fans a custom subject/body to an event's registrants over in-app bell (`broadcastNotification` CUSTOM audience) and/or email (`sendBulk`, `admin_mail` category → `mailingEnabled` guard); audience = all\|participants\|guests\|attended\|absent (+`dayNumber` for multi-day), 2000-recipient cap, audit `EVENT_MESSAGE_REGISTRANTS`. `POST /:id/feedback-poll` (same creator-or-admin gate) auto-creates + publishes a feedback `Poll` linked via `Poll.eventId` (idempotent — reuses an existing linked published poll), deadline = event end (or start) +24h (fresh now+24h if already past); `sendNow=true` (default) sends bell+email now to the chosen audience (2000-recipient cap) and reserves `Event.feedbackSentAt` so the S-10 scheduler skips it; `sendNow=false` defers to S-10 (attendees, ~2h after end) — but an **already-ended** event sends now regardless (deferring would be a silent no-op since the S-10 window only fires after end). **Idempotent once-only:** the send is gated by an atomic `feedbackSentAt`-null→now `updateMany` reservation, so a repeat click (or scheduler-then-manual) skips delivery and returns `alreadySent`; P2002 slug collisions on concurrent first-create reuse the existing linked poll. Response returns `sent`/`scheduled`/`alreadySent`, audit `EVENT_FEEDBACK_POLL_ENABLED` |
...
| `/api/certificates/*` | Mixed | `POST /generate` + `POST /bulk` accept an optional `issuedAt` (+`backdateReason`) — **PRES/SA only**, else 403. See Backdated Records below. |
| `/api/signatories/*` | Admin | |
| `/api/upload/*` | Yes | `POST /image` (CORE_MEMBER+) uploads to Cloudinary + returns `{url,publicId,bytes,width,height,format,filename}` — **no DB persistence**; the image-library gallery is **localStorage-only** (client owns history, no image link saved server-side). **Stores the ORIGINAL** (no incoming/destructive transformation — a transformation on upload would re-encode + discard the original; optimisation is delivery-time via `processImageUrl` `q_auto`/`f_auto`). `+/history` (legacy `UploadedImage` reader, no longer written to — serves only pre-change rows); `+/streak-card` (S-03 — any authed user, dedicated `streak-cards/` folder, **no** UploadedImage gallery row) |
| `/api/network/*` | Mixed | |
| `/api/audit-logs/*` | Admin (PRESIDENT/superAdmin gate on the audit log page) | `DELETE /retention?days=N` (N min 30, default 90) deletes AuditLog rows older than N days — PRES/superAdmin only (403 inside the ADMIN gate). **Automatic retention** is now available (opt-in): set `AUDIT_LOG_RETENTION_DAYS` and the 6h reminder tick's `pruneOldRecords` batch-deletes older rows; unset = manual-only (this endpoint). |
| `/api/mail/*` | Admin | |
...
| `/api/backdate/*` | **PRES/SA only** | Retroactive event records. `GET /event/:eventId` (snapshot: registrations + day attendance + guest invitations) · `POST /event/:eventId/registration` (retro-add PARTICIPANT or GUEST with a backdated `timestamp`; GUEST also gets an ACCEPTED `EventInvitation`; optional `attendedDays`) · `PATCH /registration/:registrationId/attendance` (retro mark/unmark one day) · `DELETE /registration/:registrationId` (removes the registration **and** cleans up its invitation, which the generic `events.ts` delete leaves dangling). Router-level `requirePresidentOrSuperAdmin` middleware — `requireRole('ADMIN')` admits PRESIDENT, so that inline gate is the real authority check. Sends no email/bell/socket. |
| `/api/indexnow` | Admin | |
...
### EventRegistration
`id, userId, eventId, timestamp, customFieldResponses(JSON)?, reminderSentAt?, attendanceToken?(unique), attended(default false), scannedAt?, manualOverride(default false), registrationType(RegistrationType, default PARTICIPANT), backdatedBy?(actor snapshot — set when `timestamp` was backdated by the retro console), invitation?` · unique `[userId,eventId]` · index `[eventId,attended]`, `[eventId,registrationType,attended]`.

### EventInvitation
`id, eventId, inviteeUserId?, inviteeEmail?, inviteeNameSnapshot?/Designation?/Company?, role(default "Guest"), customMessage?, status(InvitationStatus), certificateEnabled(default true), certificateType(default SPEAKER), invitedById, invitedAt, respondedAt?, revokedAt?, emailSent, emailSentAt?, lastEmailResentAt?, registrationId?(unique), backdatedBy?(actor snapshot — set when `invitedAt`/`respondedAt` were reconstructed retroactively), createdAt/updatedAt` · unique `[eventId,inviteeUserId]` · `EXPIRED` derived at read time.

### DayAttendance
`id, registrationId, dayNumber, attended(default false), scannedAt?, scannedBy?, manualOverride(default false), backdatedBy?(actor snapshot — set when `scannedAt` was backdated; **cleared** on unmark and on a non-backdated re-mark, since it describes the timestamp currently on the row), createdAt/updatedAt` · unique `[registrationId,dayNumber]`.
...
CHECKs Postgres now holds (formerly JS-only): `events_team_size_ck` (team_min ≤ team_max), `events_days_ck` (event_days 1–10), `events_capacity_ck` (capacity NULL or ≥ 0), `quiz_q_timelimit_ck` (time_limit_seconds 5–120). Plus `users_email_lower_ux` unique expression index on `lower(email)` (A8) — **not representable in schema.prisma; Prisma 5.x doesn't introspect expression indexes, so it lives outside Prisma's model (no drift, no managed drop). Re-verify on any Prisma upgrade.** Same pattern: `problem_submissions_qotd_agg_ix` (migration `20260701120000`) — partial covering index `(context_key, user_id) INCLUDE (score, submitted_at, verdict) WHERE context_type='QOTD'` keeping the QOTD total/around-me leaderboard aggregates index-only as submissions grow; partial+INCLUDE is also Prisma-invisible. Enum conversions used data-preserving `USING` casts (never Prisma's default DROP+ADD COLUMN). Two additive read indexes (migration `20260718120000`, idempotent `CREATE INDEX IF NOT EXISTS`): `audit_logs_entity_entity_id_timestamp_idx` (the notification bell's per-user system-events query filtered `entity + entity_id`, ordered by timestamp — the existing `[entity,timestamp]` index led with the wrong columns) and `problem_submissions_submitted_at_idx` (the admin AC-rate·7d `groupBy` was a bare `submitted_at` seq scan). Both are represented in schema.prisma `@@index`.
...
ProblemLanguage: PYTHON|JAVASCRIPT|CPP|C|JAVA
...
- **Crash recovery (S6, admin toggle `Settings.quizSnapshotEnabled` — "Quiz crash recovery" in Admin → Settings; env `ENABLE_QUIZ_SNAPSHOT` = emergency force-ON; default off):** live rooms snapshot to local sqlite every 10s; on boot they restore as **paused** (players marked disconnected, counters recomputed, host resumes via the normal resume path). Recovery guards: snapshot ≤30min old, quiz still WAITING/ACTIVE in DB, room not already re-hydrated; unclaimed recoveries self-ABANDON via the existing 10-min empty-room timer. The scheduler starts only AFTER boot recovery (its prune would otherwise delete the rows recovery reads).
...
| `answer_result` | s→c | unicast | per-player correct/points/streak; when `QUIZ_FOLD_RANK_IN_RESULT=true` ALSO carries optional `rank`/`totalPlayers` for sockets that advertised `foldedRankOk: true` in `join_quiz` (capability handshake) — those players then skip the duplicate `my_rank_update`; non-answerers + older bundles keep it (HC #9 intact in every configuration; flag default off = pre-fold behavior) |
...
- **Judge frame protocol (security-critical — [utils/judgeFrames.ts](apps/api/src/utils/judgeFrames.ts), unit-tested):** every harness reports each test as `__JUDGE_<nonce>:<testId>:<status>:<runtimeMs>:<base64>` plus one `__end` sentinel. Two properties make the stream trustworthy and **both are load-bearing**: (a) the **nonce is per-run and delivered out of band** as the first stdin line (`__NONCE=`) — it is NEVER embedded in the generated source, so a submission can't recover it by reading its own program file; (b) **first frame wins and a duplicate testId ⇒ `tampered`**, since the genuine frame is always emitted, so any forgery collides. A tampered run returns `JUDGE_ERROR` + `needsReview` and — unlike a genuine judge outage — **does NOT refund** the submit cap or daily quota. A missing `__end` sentinel (submission killed the process, e.g. `System.exit`) also returns `JUDGE_ERROR` rather than being scored. This replaced a bare `__JUDGE:` prefix parsed **last-wins**, under which a submission could print its own frames to fd 1 from a deferred hook (`atexit` / JVM shutdown hook / `setTimeout`) after the harness emitted the real ones and be marked **ACCEPTED without solving anything** (verified exploitable on Python, Java and JavaScript; C++ resisted because it already forked). **Per-test isolation is the real defence** — Python and C++ `fork()` with `dup2`'d pipes so user code never holds the real stdout; Java keeps the nonce in a `main()` local (unreachable by reflection); the JS `vm` sandbox is allowlist-only. `scrubHarnessInternals()` strips harness paths/frames from anything a student sees.
- **Judge pipeline:** `problemsCore.ts` → `codeJudge.ts` → per-request provider from [utils/executionRouting.ts](apps/api/src/utils/executionRouting.ts) (`Settings.codeExecutionProvider`: `wandbox` | `godbolt` | `codebox` | **`balanced`** = local-first: CodeBox wins ties/quiet periods, least-loaded otherwise; JS never routes to godbolt (no JS runtime); infra-failing host cooled down 45s) → CodeBox directly for `codebox`, else CF Worker (`workers/executor.js`), **with the other providers as automatic fallback** (see Execution Resilience below). Concurrency is **per provider per lane** (submit 5 / testrun 10 each, env-tunable `JUDGE_SUBMIT_CONCURRENCY`/`JUDGE_TESTRUN_CONCURRENCY`), so balanced mode ~triples judge throughput. Run responses carry the serving `provider`; problem run/submit results surface it for the "via X" badge (gated by `showExecutionSource`).
- **Judge admission (fast-fail busy):** when a lane's total queue passes its cap (submit 80 / testrun 120 waiters), `assertJudgeHasCapacity` in `problemsCore.ts` throws **503 `JUDGE_BUSY` BEFORE the attempt/quota is consumed** (and before anything is persisted — no review-queue noise). The stress harness (`npm run stress:judge`, [scripts/stress/](scripts/stress/run-stress.mjs)) exercises this whole path against simulated upstreams with the REAL worker code.
...
- **Batch-ceiling abort is a JUDGE_ERROR, not a student TLE:** `codeJudge.ts` aborts its own upstream call at `EXECUTION_TIMEOUT_MS` (15s, interpreted) / `COMPILED_EXECUTION_TIMEOUT_MS` (30s, compiled). That ceiling covers the WHOLE batch, while the per-test limit is enforced inside each harness (which frames `TIMEOUT` per test). The abort used to return `TIME_LIMIT_EXCEEDED`, blaming correct code for a batch that was merely slow in aggregate; it now returns `JUDGE_ERROR` (refunds cap + quota, flags `needsReview`). **Budget when authoring:** per-test timeout x test count must stay under the ceiling.
...
- **QOTD today/summary read cache ([apps/api/src/utils/qotdTodayCache.ts](apps/api/src/utils/qotdTodayCache.ts), factory + prod singleton, unit-tested):** the two per-request-identical shared reads — `GET /today`'s published today-row (findFirst+problem) and `GET /history/summary`'s published-QOTD `{id,problemId}` list — are served from an **IST-date-keyed 60s single-flight** cache (new IST day = new key ⇒ midnight rollover needs no TTL wait; rejections never cached; O(1) memory). Per-user hasSubmitted/hasSolved stay **live** queries; the staff `?includeUnpublished` view **bypasses** the cache. `GET /today` drops 2→1 queries for a logged-in user (1→0 anonymous). `invalidateQotdTodayCache()` fires on every go-live/hold/content change: create-and-publish-now, `POST /:id/publish`, `POST /:id/hold`, the `PUT /:id` go-live branch, `DELETE /:id` (when published), and the auto-publish timer fire in `scheduler.ts` `publishDueQotd`. Deliberately NOT invalidated: reopen/close-reopen (past-QOTD `reopenedAt` only — not in either cell) and publish-practice/unpublish-practice (linked-`Problem` flip, past days only).
...
- **Round + registration read cache ([apps/api/src/competition/roundCache.ts](apps/api/src/competition/roundCache.ts), factory `createRoundCache` + prod singleton, unit-tested — `roundCache.test.ts`, 12 tests):** during a live contest 200-250 students re-poll the same two DB reads every ~15s (GET `/:roundId` status, proctor heartbeat/me, save/submit + leaderboard gates all call `ensureRegisteredForRound`). The near-static **round row + linked-problems join** is served **single-flight, 10s TTL, bounded 64 entries** — all pollers share ONE query per 10s window. The **registration check is POSITIVE-ONLY cached 30s** (capped 2000 entries, oldest-evicted): a **miss always re-queries live** (a brand-new registrant is never wrongly blocked); a deleted registration can read valid for ≤30s **on read-only polls + the heartbeat only** — IMAGE_TARGET `/save`+`/submit` re-check fresh (`liveRegistration: true`) and the authoritative DSA submit gate `problemsCore.validateProblemContext` stays fully live. **Every round/problem write invalidates** the round entry: start, lock, judging, finish (both branches), extend, PUT edit, DELETE, publish-as-practice, and the `autoLockRound` timer path (boot recovery flows through `autoLockRound`). Net effect: the two hottest per-poll queries (250 × every 15s) collapse to ~one query per 10s/30s window platform-wide — direct relief on the small owner-tuned pool (each query is a network round-trip, the query itself <1ms).
...
- **Realtime — relay on the playground server (Phase H, was on the main API in G):** the `/competition` Socket.io namespace lives on the **idle playground execute-server** ([apps/playground/execute-server.js](apps/playground/execute-server.js): auth via shared JWT, rooms gated by `pg` registration / admin role) so the main API sheds persistent-connection memory during contests. The main API stays authoritative for all writes + **all scoring**: [competitionRealtime.ts](apps/api/src/competition/competitionRealtime.ts) computes payloads (team-aware via `computeContestLeaderboard`, freeze-aware) and **`relayEmit`s** them as fire-and-forget HTTP `POST /internal/contest-emit` (shared `INTERNAL_API_SECRET`); the relay fans out to rooms (`round:<id>`, `round:<id>:admin`, `round:<id>:user:<uid>`). The bounded per-active-round store (first-solve map + 1/sec leaderboard throttle, evicted on finish) stays on the main API. Events: `contest:leaderboard` (throttled, freeze-aware), `:submission`, `:firstSolve`, `:clarification`, `:status`, `:round` (live extend), `:proctor`, `:violation`, `:participant`. Clients (arena `useContestSocket` [apps/playground](apps/playground/src/hooks/useContestSocket.ts) → `VITE_API_URL`; admin monitor `useContestAdminSocket` [apps/web](apps/web/src/hooks/useContestSocket.ts) → **`settings.playgroundApiUrl`** (served at runtime from the API's `PLAYGROUND_API_URL`) → `VITE_PLAYGROUND_API_URL` build-fallback) connect to the relay; **Phase E REST polling is the fallback** when the relay is unconfigured/down. Force-logout also hits `POST /internal/disconnect-user`. `buildDsaLeaderboard` is shared from [contestScoring.ts](apps/api/src/utils/contestScoring.ts).
- **Contest priority mode (Phase G, [contestMode.ts](apps/api/src/competition/contestMode.ts)):** while ≥1 round is ACTIVE the 6h reminder/feedback/retention-prune tick and the DB keep-alive ping are skipped (counter inc on start, dec on the ACTIVE→LOCKED transition only, seeded by `recoverActiveRounds`); user-facing services stay up, auto-restored when the last round ends.
- **Proctor lockdown (Phase G + hardening):** a proctored round enforces **fullscreen** (exit) + **clipboard block** (copy/cut/paste) + blocks **right-click, dev-tools (F12 / Ctrl+Shift+I·J·C / Cmd+Alt+I·J·C), view-source (Ctrl+U), print (Ctrl+P), save (Ctrl+S)** + a `beforeunload` guard, all via `useProctor` options. **Instant-lock kinds** = `COPY_PASTE` (copy/cut/paste, `detail` distinguishes) + `FULLSCREEN_EXIT`; they get a per-participant **budget** server-side ([competition.ts](apps/api/src/routes/competition.ts) `INSTANT_VIOLATION_BUDGET`, **default 1**): the first is counted + warned (arena toast; `POST /proctor/violation` returns `{warning, remaining}`), the **2nd locks** (1 warning, then lock). The tally is scoped to violations **after the participant's last `unlockedAt`**, so an admin **Unlock** actually restores the warning — counting the whole round made an unlock a no-op (the next copy/cut instantly re-locked). The full `CompetitionViolation` log is untouched; only the lock decision resets. The blocked dev-tools/right-click/print/save kinds report as **`OTHER` + `detail`** and are **logged + counted but never auto-lock** (the invigilator sees them in the live log and locks manually) — avoids false locks from a reflexive Ctrl+S. **Tab-away still locks on its first trip** (10s on-screen grace). Every violation is logged + counted regardless. `emitViolation` carries `userName` + `detail` so the admin log row is self-sufficient. The DSA arena is lock-only (drafts preserved), IMAGE_TARGET still auto-submits-then-locks. Admin lock/unlock pushes ride `contest:proctor` → `applyProctorPush` applies instantly (heartbeat poll is the ≤15s fallback).
- **Lifecycle on finish (no lock/unlock after end):** `PATCH /finish` (both branches) **clears every `CompetitionParticipantState.locked`** in its txn (nothing left stuck — "everything zero"), and `POST /proctor/lock` (gated to ACTIVE/LOCKED) + `POST /proctor/unlock` (rejected once FINISHED) are **status-gated** so an ended round can't be locked/unlocked. The arena already gates solving on `status==='ACTIVE'`, shows a terminal "contest ended" banner, and stops its leaderboard/clarification polls + socket when FINISHED.
- **Rate limits:** save 12/min, submit 5/min, user-keyed.
- **Contest-day ops (200-250 students):** before a big contest — (a) raise `Settings.playgroundDailyLimit` (default **100/day**; CONTEST runs+submits consume it, heavy debuggers hit the wall mid-round; **~300-500** recommended, 15s-cached, no deploy); (b) raise per-problem submit caps via the round-wide `POST /:roundId/raise-cap` (default `defaultSubmitCap=5`/problem is tight); (c) tell students to **sign in before the start window** (a synchronized ~200-login bcrypt burst saturates the 0.5-vCPU box for ~2min); (d) prefer `codeExecutionProvider=balanced` (local CodeBox first, cloud spillover) + the socket relay (`PLAYGROUND_API_URL`) over polling.
...
POST (Admin create) · GET `/event/:eventId` (optional auth — admin sees all, public sees metadata) · GET `/event/:eventId/results-summary` (Admin, cert flow) · GET `/:roundId` (User) · PATCH `/start|lock|judging|finish` (Admin) · POST `/save` (User, rate-limited) · POST `/submit` (User, rate-limited; idempotent per `[roundId,userId]`) · GET `/my-submission` (User) · GET `/submissions` (Admin) · PATCH `/score/:submissionId` (Admin) · GET `/results/export` (Admin, ExcelJS) · GET `/results` (public when FINISHED) · POST `/publish-as-practice` · POST `/raise-cap` · PUT/DELETE round (Admin, cascades). **Proctoring (Phase C):** `POST /:roundId/proctor/violation` (User — records a `CompetitionViolation` + locks the participant on a proctored live round; the client force-submits its draft first), `POST /:roundId/proctor/heartbeat` (User — `lastSeenAt` + returns lock state; the arena's lock poll), `GET /:roundId/proctor/me` (User — self lock state), `POST /:roundId/proctor/unlock/:userId` + `POST /:roundId/proctor/lock/:userId` (Admin — release/freeze a participant, audited, **status-gated** — see Lifecycle on finish). **Lock enforcement:** IMAGE_TARGET `/save`+`/submit` and the DSA run/submit path (`validateProblemContext` CONTEST branch, gated on `requireActiveContest` so the read-only problem view isn't blocked) reject a locked participant with 403. Client engine: `useProctor` ([apps/playground/src/hooks/useProctor.ts](apps/playground/src/hooks/useProctor.ts)) — visibilitychange + window-blur (ignores focus into our preview iframe) → 10s away → auto-submit then violation → locked overlay; clipboard/dev-tools/print blocks (see Proctor lockdown); heartbeat re-arms on admin unlock. **Live monitor / leaderboard / clarifications:** `GET /:roundId/leaderboard` (User registered — live DSA board via shared `buildDsaLeaderboard`; non-admins get a full freeze `{frozen:true}` in the final `leaderboardFreezeMinutes`; admins always live), `GET /:roundId/clarifications` (User), `POST /:roundId/clarifications` (Admin broadcast), `GET /:roundId/monitor` (Admin — participants merged from `CompetitionParticipantState` (online via `lastSeenAt`, lock, violations) + DSA score, plus a recent-submission feed, a **`recentViolations`** slice for the live-log seed, and `round.startedAt`/`duration` for the countdown). **Admin UI: `CompetitionMonitor`** ([apps/web/src/pages/admin/CompetitionMonitor.tsx](apps/web/src/pages/admin/CompetitionMonitor.tsx), route `/admin/competition/:roundId/monitor`) — **socket-first live console**: consumes the `/competition` relay payloads via `useContestAdminSocket` (typed per-event callbacks + `connected` flag) to drive a **human-readable live event log** (`lib/contestViolations.ts` `violationLabel` maps `HIDDEN→"Switched tab"`, `COPY_PASTE→"Pasted code"`, `OTHER+detail→"Opened dev tools"`…) and patch rows in place — the main-API `/monitor` poll is only a **slow reconcile when connected (30s)**, fast (8s) when the relay is down, and `refetchIntervalInBackground:false`. Features: participant table with **status-gated lock/unlock** (hidden once FINISHED) + **Unlock all**, **per-participant + round-wide raise-cap**, **extend time**, a **submission dialog** (lazy-loads `getCompetitionSubmissions` for code + inline score/verdict override → DSA `adminOverrideSubmission`, IMAGE `scoreCompetitionSubmission`), clarification broadcaster, connection/online/locked/time-left stat strip, plagiarism review. Arena shows **Problems + Leaderboard** tabs; **clarifications moved to a floating corner window** that **auto-opens** when an admin broadcasts.

### Frontend
`AdminCompetition` (`/admin/competition`) — round editor also configures the redesign settings: ranking model (BEST_SCORE/ICPC), event-final weight, leaderboard freeze minutes, proctored toggle, and **difficulty-seeded per-problem weights** with a live normalized-share % preview. The Easy/Medium/Hard **seed presets are editable + persisted** on the round (`difficultyWeights`, default 100/200/300) — a newly-added problem's weight seeds from them, and the admin's tuning survives a reload; the raw per-problem weight still lives on each problem's `points`. `POST`/`PUT /competition` accept + the editor now sends these (`finalWeight`, `proctored`, `penaltyModel`, `leaderboardFreezeMinutes`, `difficultyWeights`, per-problem `points`). FINISHED rounds expose a **Certificates** action + the event-final dialog (`FinalStandingsDialog`, now with a top-3 podium) an **Issue winner certificates** CTA — both deep-link to `/admin/events/:eventId/attendance?tab=certificates[&competition=<roundId>]`. · `CompetitionJudge` (`/admin/competition/:roundId/judge`) · `CompetitionSolvePage` (`/competition/:roundId/solve/:problemId`, DSA) · `CompetitionResults` (`/competition/:roundId/results`) — **redesigned onto dashboard-v2 (dash primitives, rust/accent tokens; legacy amber retired)**, preserves DSA standings (ICPC penalty + breakdown + runtime + members) and the IMAGE_TARGET podium + auto/manual standings. · `EventCertificateWizard` consumes `getCompetitionResultsSummary()` to bulk-issue WINNER certs via `competitionCertificateUtils.ts`; **reads `?competition=<roundId>` to jump straight into the competition flow** pre-selecting that round (specific-round strategy).

---

## Backdated Records (PRES/SA)

Retroactive event records, so a certificate for something that already happened can carry an
honest date (the driving case: issuing a SPEAKER certificate today for a talk given two years
ago, dated to that talk). Restricted to **PRESIDENT / super admin** — a plain ADMIN cannot
rewrite history.

- **Policy lives in one pure module:** [apps/api/src/utils/backdate.ts](apps/api/src/utils/backdate.ts) (`resolveBackdate`, unit-tested — 13 cases in [backdate.test.ts](apps/api/src/utils/backdate.test.ts)). Bounds: never in the future (a <5 min fast client clock is **clamped**, not rejected), never before `BACKDATE_FLOOR` (2015-01-01 — catches a mistyped year), and for an event-linked record never before `event.startDate − 24h` (the day of slack absorbs UTC/IST skew). A date within `BACKDATE_MIN_OFFSET_MS` (1h) of now resolves as **not** a backdate, so opening the control and picking today doesn't stamp false retro markers. `now` is always injected — the module never reads the clock.
- **Only the record's OWN effective date moves.** `Certificate.issuedAt`, `EventRegistration.timestamp`, `DayAttendance.scannedAt`, `EventInvitation.invitedAt`/`respondedAt`. Every `createdAt` and every `AuditLog.timestamp` keeps true wall-clock time, so a reconstruction is always distinguishable and always traceable. Backdated writes use distinct audit actions (`CERTIFICATE_GENERATE_BACKDATED`, `CERTIFICATE_BULK_GENERATE_BACKDATED`, `ATTENDANCE_EDIT_BACKDATED`, `ATTENDANCE_MANUAL_BACKDATED`, `BACKDATE_REGISTRATION_CREATE`/`_DELETE`, `BACKDATE_ATTENDANCE_EDIT`) so the audit log is filterable.
- **`backdatedBy` is a plain-string actor snapshot** on Certificate / EventRegistration / DayAttendance / EventInvitation (audit A3 — must survive the acting admin's deletion). Migration `20260807120000_backdated_records`, additive + `IF NOT EXISTS`.
- **Certificates:** `POST /api/certificates/generate` and `/bulk` take `issuedAt` + `backdateReason`; non-PRES/SA gets 403, an out-of-bounds date gets 400. `issueOneCertificate` resolves the date **once, outside** the certId-collision retry loop (a retry must not shift the date by a PDF render). Bulk resolves it once for the whole batch — a batch straddling two dates would be indefensible. **The PDF renders no date at all** (owner decision) — `CertData.issuedAt` is carried but never drawn, so backdating is invisible on the artwork and visible everywhere else.
- **Two bugs this closed:** `email.ts sendCertificateIssued` hardcoded `new Date()` for its "Issued On" line (it now takes the real `issuedAt`, and **resend replays the stored one**); and the notification bell windowed + sorted + read-flagged certificates on `issuedAt`, so a backdated certificate would arrive **pre-read and buried years down the feed** — [notifications.ts](apps/api/src/routes/notifications.ts) now windows/sorts/read-checks on `createdAt` and surfaces `issuedAt` in the item body.
- **Attendance:** `PATCH /api/attendance/edit/:registrationId` keeps its 24h correction window for CORE_MEMBER/ADMIN; **outside** that window it falls through to the PRES/SA backdate path instead of 400ing. `POST /manual-checkin` takes an optional `scannedAt` (PRES/SA only). `editDayAttendance` **clears** `backdatedBy` on unmark and on a non-backdated re-mark — the marker describes the timestamp currently on the row.
- **Retro registration / guest:** [apps/api/src/routes/backdate.ts](apps/api/src/routes/backdate.ts) (see the API Routes table). Reuses `createEventRegistrationInTx` (which gained `timestamp`/`backdatedBy`) so a retro registration gets the same attendance token + seeded DayAttendance rows as an organic one. A GUEST also gets an **already-ACCEPTED** `EventInvitation` (a PENDING invitation on a finished event would record something that never resolved). Idempotent on re-submit: an existing registration is **reused, not rewritten** — its `timestamp` is deliberately kept (it may be a real organic registration moment, and overwriting it would destroy a true record to satisfy a retro form), while its `registrationType` **is** reconciled to the requested one (leaving it would let an ACCEPTED GUEST invitation point at a row still marked PARTICIPANT, which the organic accept flow refuses outright and which would surface a speaker in every `participantsOnly` count). Both the API message and the console toast say the date was kept and that removing + re-adding is how to change it. On the invitation-update branch `emailSent` is left alone (a prior invitation may genuinely have mailed the guest) and `revokedAt` is cleared (ACCEPTED + revoked is self-contradictory). **Capacity, registration windows and `maxEventsPerUser` are deliberately not enforced** — they govern live intake into an upcoming event, and a finished event's registrations already don't count toward the active-event limit.
- **No email, no bell, no socket** from the retro router. These records describe the past; announcing them would be nonsense. The certificate is the one thing the member should hear about, and that keeps its own `sendEmail` toggle.
- **UI:** unlisted `/admin/backdate` console (`AdminBackdate`, inside `SuperAdminOrPresidentRoute`, **not in the sidebar or command palette**) + a collapsed-by-default `CertificateBackdateControl` ([apps/web/src/components/certificates/](apps/web/src/components/certificates/CertificateBackdateControl.tsx)) shared by `EventCertificateWizard` and `GenerateCertificateDialog`, which renders `null` for anyone who isn't PRES/SA. Hiding is presentation; the server re-checks authority on every request.

### Rejected proposals
- Printing the issue date on the certificate PDF (owner decision — would change the appearance of every existing certificate template).
- Backdating `AuditLog.timestamp` alongside the record. The audit trail is the only thing that makes backdating safe; moving it would defeat the feature's own accountability.
- A `Certificate.createdAt` column — it already existed, which is what makes the bell fix a one-liner.
...
**`CertData`:** `recipientName, eventName, type, position?, domain?, description?, certId, issuedAt, signatoryName, signatoryTitle?, signatoryImageUrl?, facultyName?, facultyTitle?, facultySignatoryImageUrl?, codescrietLogoUrl?, ccsuLogoUrl?`. `formatPosition()` "1"/"1st" → "First Place". QR links `FRONTEND_URL/verify/:certId`. Uploaded to Cloudinary → `Certificate.pdfUrl`. Public verify: `GET /api/certificates/verify/:code` (**does not expose `pdfUrl`**). `viewCount` is **buffered in memory and flushed every 30s as one set-based `UPDATE … FROM (VALUES …)`** ([utils/certificateViewCounter.ts](apps/api/src/utils/certificateViewCounter.ts)) — this endpoint is public + unauthenticated, so a write per read was pure amplification on the pooled Neon link. Buffer is bounded (500 certIds, early flush on overflow) and drained by the shutdown path, so a normal redeploy loses nothing; a crash loses at most one 30s window of a non-audit counter.
...
- [NotifMenu.tsx](apps/web/src/components/dashboard/NotifMenu.tsx) — bell popover, All/Unread tabs, refetch every 30s when open, "Mark all as read". Clicking **any** item opens [NotificationDialog.tsx](apps/web/src/components/dashboard/NotificationDialog.tsx) (full message rendered as Markdown with clickable links + an "Open …" action that forwards to its link) instead of navigating away directly. The dialog stamps `data-dashboard`/`data-accent` on its portaled content so the `--ds-*`/`--accent` tokens resolve.
...
- Execute server: `apps/playground/execute-server.js` (plain JS, port 5002). Proxies CF Worker → admin-selected provider (`Settings.codeExecutionProvider`, 60s cache, instantly flushed via `POST /internal/flush-caches` on settings save; `balanced` is resolved to ONE concrete host per execution — CodeBox first when healthy/under its inflight cap, else least-loaded remote; JS/TS pinned to Wandbox/codebox (never godbolt), 45s infra cooldowns — before forwarding as `body.provider`). Python: Pyodide (browser, no server call). Daily limits via `Settings.playgroundDailyLimit` (default 100/day). Tables: `PlaygroundDailyUsage`, `PlaygroundLimitReset`, `PlaygroundLimitResetRequest`. **Authed CRUD routes (snippets/prefs/session) carry a hand-rolled user-keyed request limiter** (`crudRateLimit`, 120/user/60s, bounded map swept at 2×window) — they previously had no per-window cap (`/api/execute` keeps its own daily-quota/IP metering, which now runs BEFORE the `checkSecurityPatterns` regex so an anonymous flood can't burn regex CPU).
- **Contest relay + plagiarism offload (Phase H):** because this box is mostly idle, it also hosts the contest `/competition` Socket.io namespace (auth via shared JWT; rooms gated by `pg` registration / admin role) and three internal endpoints the main API calls server-to-server, gated on a shared secret that is **`INTERNAL_API_SECRET` if set else derived from `JWT_SECRET`** (`deriveInternalSecret()`, the exact mirror of the main API's `getInternalApiSecret()` — so no separate secret needs configuring): `POST /internal/contest-emit` (dumb relay → fan-out to rooms), `POST /internal/disconnect-user` (force-logout), `POST /internal/plagiarism` (runs the O(N²) similarity from `plagiarism.js`, the JS mirror of [apps/api/src/competition/plagiarism.ts](apps/api/src/competition/plagiarism.ts)). No scoring/judging here — only connections + the plagiarism batch. Deps: `socket.io`. **The secret check runs as app-level middleware BEFORE the 12mb `express.json` parser**: this service is public, so gating inside the handlers let an unauthenticated caller force a 12 MB allocation + blocking `JSON.parse` per request and stall the relay's event loop. CORS is not a defense here (origin-less requests are allowed, and CORS never blocks non-browser clients). A valid secret is **trusted and bypasses rate limiting entirely** (expressed as the limiter's `skip`, so the limiter stays a declared middleware — visible to readers and to CodeQL's "authorization without rate limiting" rule, which cannot follow an imperatively-invoked limiter); only REJECTED callers hit a 60/min/IP abuse limiter. Do NOT meter trusted callers — this service sets no `trust proxy`, so `req.ip` is the edge-proxy address for every caller, which would put the main API's emits and an attacker's junk in one bucket (the leaderboard alone emits 2 POSTs/sec per ACTIVE round = 120/min), and `relayEmit` is fire-and-forget so the resulting 429s would drop pushes silently. `relayEmit` now logs non-2xx responses for exactly that reason.
...
- **CodeBox (self-hosted Judge0 engine, 4th provider):** [hiteshchoudhary/Codebox](https://github.com/hiteshchoudhary/Codebox) (MIT) runs on the VM as API (`127.0.0.1:3000`) + worker + Redis via `docker-compose.vm.yml` (their Caddy/Prometheus/Grafana services excluded — ours already exist). Docker/isolate executor only (`/dev/kvm` absent ⇒ no Firecracker); **linux/amd64 images don't run on Ampere — `docker/worker/Dockerfile` carries an arch-conditional symlink patch** (`aarch64-linux-gnu`, `java-17-openjdk-arm64`). Worker capped SAFE (`cpus: 1.0`, `memory: 2G`, `WORKER_CONCURRENCY=1`) so contest spikes queue instead of jittering the VPN. Judge0 IDs: C 50, C++ 54, JS 63, Python 71, Java 62 (no TS image — TS stays on Wandbox). Both runtimes call it directly over loopback; per-language `memory_limit` (Java 512000, else 256000). The sandbox static analyzer rejects `os.fork()`, `child_process`-style module strings, `java.lang.reflect`, `URL*` APIs and sensitive paths **even in comments** — harnesses are written accordingly (fragmented literals, MethodHandles invocation, byte-loading classloader).
### Execution Resilience (balanced triple-provider routing + worker fallback chain)
...
- **Admin-selectable mode:** `Settings.codeExecutionProvider` (`wandbox` | `godbolt` | **`balanced`**, default `wandbox`). The admin picks it in Admin → Settings (`CodeExecutionCard` → `PATCH /api/settings/codeExecutionProvider`). **`balanced` (recommended for contests)** resolves ONE concrete provider per request — least-loaded lane first, round-robin ties, JS pinned to Wandbox, a host that infra-failed cooled down 45s — in [utils/executionRouting.ts](apps/api/src/utils/executionRouting.ts) (API judge; per-provider submit/testrun semaphores + queue-cap admission live there too, unit-tested) and a JS mini-mirror inside [execute-server.js](apps/playground/execute-server.js) (playground). Both callers therefore always send the worker a CONCRETE `body.provider` — **balanced works with the currently-deployed worker, no CF redeploy required**. Fixed settings read as before: judge via `getCachedSettings()` (5-min, instant on invalidation) in [codeJudge.ts](apps/api/src/utils/codeJudge.ts), playground via its 60s `providerCache`; a fixed-but-cooling provider is pre-routed to the healthy one (same semantics the worker fallback would apply, minus the stall).
- **Worker provider telemetry (needs the next manual worker deploy):** `runWithChain` now stamps `judge_provider` (which upstream served) + `judge_fallback` (lead provider infra-failed) on the result; both callers feed these into their health cooldowns when present and degrade gracefully when absent. The worker also accepts `provider:'balanced'` itself (per-isolate round-robin) and `env.WANDBOX_URL`/`env.GODBOLT_BASE` endpoint overrides (used by the stress harness to run the REAL worker against simulated upstreams).
...
- Shares JWT secret with main API, reads `scriet_session` cookie. **`optionalAuth` also enforces the main API's two DB-side revocation gates** — `is_deleted` and the `token_version` force-logout watermark — via a 30s bounded cache (mirrors [utils/userAuthCache.ts](apps/api/src/utils/userAuthCache.ts)). Previously it verified only the JWT signature, so an admin force-logout was cosmetic here: the target kept execution/snippets/session for the token's remaining 7 days and simply reconnected after their sockets were dropped. `POST /internal/disconnect-user` now also invalidates that cache so a kick applies on the next request rather than after the TTL. The check **fails OPEN** on a DB fault (it runs on every request; prior behavior was no check at all, so this is never worse than the status quo). The `/competition` socket `join` additionally compares `token_version` against the handshake claim.
...
- **Viewport height:** every full-screen shell uses **`.h-app`**, never `h-screen`. `100vh` on mobile is the *expanded* viewport (it ignores the URL bar) and pushes bottom bars off-screen; `.h-app` cascades `100vh → 100svh → 100dvh` (index.css, plus `.max-h-sheet` / `.pb-safe` / `.no-scrollbar`). ⚠️ **`.pb-safe` is an unlayered rule emitted after `@tailwind utilities`, so it BEATS `pb-*`/`py-*` at equal specificity rather than adding to them** — only put it on an element with no other bottom padding, else use `pb-[calc(1.5rem+env(safe-area-inset-bottom,0px))]`. Note the app pads for the *bottom* inset only: `apple-mobile-web-app-status-bar-style` is deliberately `default`, not `black-translucent`, because nothing pads for `safe-area-inset-top`.
- **`MobileSheet`** ([components/ui/mobile-sheet.tsx](apps/playground/src/components/ui/mobile-sheet.tsx)) — the body scroll lock is **ref-counted at module scope**, not a per-instance snapshot: the arena can auto-open a clarification sheet over the problem picker, and two instances each restoring their own "previous" overflow leaves the page permanently unscrollable. Also moves focus into the dialog and restores it to the opener on close (the cleanup checks for *orphaned* focus, since React has already detached the panel by the time a passive cleanup runs), dismisses on `pointerdown` (touch browsers suppress the synthetic mousedown once a touch sequence is preventDefault'd), and takes a **required** `title` — it is the dialog's accessible name.
...
- **Monaco touch profile:** `getEditorOptions({ touch, fontSize })` ([lib/monacoEditor.ts](apps/playground/src/lib/monacoEditor.ts)) layers `MOBILE_MONACO_OVERRIDES` on the base options — no folding/glyph margin/overview ruler (reclaims width), no hover/context-menu popups, `scrollbar.vertical/horizontal: 'visible'` (a phone can't hover to reveal an auto-hiding thumb), and `fontSize` floored at **`TOUCH_MIN_FONT_SIZE`** (16 — the UI's own +/- controls clamp to the same constant, so they can't change state the floor silently undoes). **Enter always makes a newline and Tab always indents**: `acceptSuggestionOnEnter`/`tabCompletion` are not sufficient on their own, because an OPEN suggest widget binds Tab to accept-suggestion regardless — so the widget itself is off on touch (`quickSuggestions`/`suggestOnTriggerCharacters`/`wordBasedSuggestions`/`inlineSuggest`).
- **Editor lifecycle — never unmount Monaco to switch panes.** The undo stack lives on the *model*, and unmounting disposes it (`@monaco-editor/react` defaults `keepCurrentModel:false`), taking the caret and scroll position too. A Test Run / Submit auto-flips the solver to its results pane, so unmounting there would wipe a contestant's history on **every run**. All three surfaces (`QOTDSolverShell`, `PlaygroundPage`, `CompetitionPage`) therefore **hide** the code pane (`hidden` + `display:none`) instead of conditionally rendering it, and every editor sets `keepCurrentModel` so even a genuine unmount (a breakpoint flip) preserves history. `automaticLayout` re-measures when the pane returns.
- **`useEditorHistory` commands return `boolean`** — whether they reached a live editor. `handleMount` registers an `onDidDispose` that nulls the ref and clears `isReady`/`canUndo`/`canRedo`, so controls disable rather than silently no-op, and callers (`resetCode`, the solver's Ctrl/Cmd+Shift+R) check the result instead of toasting a success that never happened. `insertText` refuses to write to a `readOnly` editor — its version-id fallback uses `executeEdits`, which ignores `readOnly` and would otherwise edit a frozen submission.
...
| Var | Req | Notes |
|---|---|---|
| `DATABASE_URL` | yes | Neon pooler. Its **`connection_limit` query param governs the Prisma pool size** (`resolvePoolMax()` in [apps/api/src/lib/prisma.ts](apps/api/src/lib/prisma.ts) reads it; the pg driver adapter ignores it otherwise, so this is the only knob). Dashboard-set, code fallback 5; owner-approved target **20** (HC #3). |
...
| `AUDIT_LOG_RETENTION_DAYS` | no | Opt-in automatic AuditLog pruning: a positive int **≥30** ⇒ `pruneOldRecords` batch-deletes AuditLog rows older than that many days (on the 6h reminder tick, ≤1×/24h). Unset / `<30` / non-numeric ⇒ disabled (compliance trail kept forever; a typo can't start deleting). The manual `DELETE /api/audit-logs/retention` is unaffected. |
| `EVENT_STATUS_INTERVAL_MS` | no | deprecated — event-status is now event-driven (timer until next boundary), no longer polled |
| `CODE_EXECUTION_PROVIDER` | no | fallback (judge + playground) when the `Settings.codeExecutionProvider` column/DB is unavailable (`wandbox`\|`godbolt`\|`balanced`\|`codebox`, default `balanced` on VM / `wandbox` upstream). Normal path reads the DB setting. |
| `CODEBOX_URL` / `CODEBOX_TOKEN` | no | local Judge0 engine (CodeBox) origin + auth for the 4th provider. Default `http://127.0.0.1:3000` + empty token. Both runtimes call it directly over loopback (never via the CF Worker); the worker stays the backstop. |
| `JUDGE_SUBMIT_CONCURRENCY` | no | max concurrent judge submits **per provider** (default 5). Balanced mode's platform total = 3×. Sized to today's per-upstream pressure — raise only with evidence the upstream tolerates it. |
| `JUDGE_TESTRUN_CONCURRENCY` | no | max concurrent judge test-runs **per provider** (default 10). Same per-provider semantics. |
| `EXECUTOR_SECRET` | no | shared secret sent as `X-Executor-Secret` to the CF Worker (M1). Already supported by the playground; the API judge now sends it too when set. Enforce by setting the same value in the Worker env. |
| `INTERNAL_API_SECRET` | no | shared secret for main API ↔ playground internal calls (contest relay emit/disconnect + plagiarism offload). **Unset ⇒ auto-derived from `JWT_SECRET`** (both services already share it) by `getInternalApiSecret()` ([utils/internalApi.ts](apps/api/src/utils/internalApi.ts)) + the JS mirror `deriveInternalSecret()` in the execute-server — so the relay needs no separate secret: setting `PLAYGROUND_API_URL` alone turns it on. Set explicitly on **both** services only to override the derived value. No security loss (forging it requires `JWT_SECRET`, already full compromise). |
| `PLAYGROUND_API_URL` | no | main API → playground execute-server origin (the `/competition` relay + `/internal/plagiarism`). The **one** value to set (on the main `codescriet-api` service) to turn the relay/offload on; a bare host is accepted (`https://` assumed). Unset ⇒ inline + polling. **Also surfaced to the web admin monitor** at runtime: `GET /api/settings/public` returns it as `playgroundApiUrl` (via `getPlaygroundRelayBase()`), so the monitor connects its socket **without a web rebuild** — set this single API env var and both the relay emit AND the admin monitor turn on. |
| `VITE_PLAYGROUND_API_URL` | no | **Optional build-time fallback** for the admin monitor's `/competition` socket origin. Normally unneeded: the monitor prefers `settings.playgroundApiUrl` (served from the API's `PLAYGROUND_API_URL` at runtime), then this build var, then dev localhost; unset everywhere ⇒ REST polling. (Playground arena uses its own `VITE_API_URL`.) |
| `PG_STATEMENT_TIMEOUT_MS` | no | per-connection Postgres `statement_timeout` on the pg adapter pool. **Default OFF (unset/0 = disabled)** — opt-in only AFTER verifying Neon's pooled endpoint (PgBouncer) honors the libpq `options` startup parameter (run `SELECT current_setting('statement_timeout')` through a client built exactly like `lib/prisma.ts` against the prod DATABASE_URL; PgBouncer may honor, silently strip, or REJECT the parameter — rejection would refuse every pooled connection). Once verified, ~8000 is the owner-approved S0 safety valve: a runaway query can no longer pin one of the 5 pooled connections indefinitely. Fires as PG `57014`, deliberately NOT retried by `withRetry`. Before tightening below 8s, batch the audit-log retention DELETE and verify a 900-player quiz persist. |
| `QUIZ_FOLD_RANK_IN_RESULT` | no | **Emergency force-ON override only** — the primary switch is the admin Settings toggle `Settings.quizFoldRankInResult` (Admin → Settings → "Quiz lean reveals"), read per reveal via the sync `peekCachedSettings()` (zero DB on the reveal path; live-toggleable). Folds a player's post-reveal rank into their `answer_result` unicast instead of a separate `my_rank_update` — ONLY for sockets that advertised `foldedRankOk` in `join_quiz` (capability handshake; old bundles keep the two-message shape). Both off = exact pre-fold behavior. |
| `WS_ENGINE` | no | Selects the Socket.io WebSocket engine on both servers (main API + playground relay). **eiows (C++ µWS fork, ≈5-8x lower per-connection memory) is now the DEFAULT** — the lever that raises the ~900-concurrent ceiling; same wire protocol, so `socket.io-client` is unaffected. `WS_ENGINE=ws` forces stock `ws` (instant, no-code rollback). eiows stays an `optionalDependency` pinned to `9.2.0`: if its native build is skipped/failed it **auto-falls back to `ws`, but LOUDLY** (an `error` log + the `wsEngine` field on `GET /health` on both services — confirm with one curl). Decision core `chooseWsEngine` is unit-tested. **Both services load it via `createRequire(import.meta.url)`** — the playground previously used a bare `require('eiows')` inside an ESM module, which threw `ReferenceError: require is not defined` on every boot, was swallowed by the fallback catch, and silently pinned the contest relay (the box that actually holds the contest sockets) to stock `ws`. Check the `wsEngine` field on `GET /health` after deploy to confirm which engine is live. |
| `WS_ENGINE_STRICT` | no | `true` makes an unavailable eiows a **boot refusal** instead of a `ws` fallback, so a regressed native build fails the DEPLOY rather than silently serving the heavier engine in prod. Use it once you've confirmed eiows builds. Default: fall back to `ws` (never blocks startup). Ignored when `WS_ENGINE=ws` (explicit opt-out wins). |
| `ENABLE_QUIZ_SNAPSHOT` | no | **Emergency force-ON override only** — the primary switch is the admin Settings toggle `Settings.quizSnapshotEnabled` (Admin → Settings → "Quiz crash recovery"), consulted per 10s tick via the sync `peekCachedSettings()` (live-toggleable, no restart). S6 crash-durable quiz snapshots: every 10s each live room is written to a LOCAL better-sqlite3 file ([quizSnapshot.ts](apps/api/src/quiz/quizSnapshot.ts)); on boot, recent (≤30min) snapshots whose quiz is still WAITING/ACTIVE in the DB are rebuilt **as `paused` rooms** (host presses Resume — the existing resume path re-arms the timer, so recovery adds zero timer code). Engine untouched (observe-only + public store API); optional native dep degrades to a warn+no-op. Covers crash/OOM, NOT redeploys (Render disk is ephemeral across deploys). Default off. |
| `QUIZ_SNAPSHOT_DB` | no | sqlite file path for the S6 snapshots (default `.data/quiz-snapshots.db`). |
...
| TTL single-flight cache (canonical — use for new cached aggregates) | [apps/api/src/utils/singleFlight.ts](apps/api/src/utils/singleFlight.ts) |
...
| Judge frame protocol (nonce, tamper detection, scrubbing) | [apps/api/src/utils/judgeFrames.ts](apps/api/src/utils/judgeFrames.ts) |
| Per-language judge harnesses | [apps/api/src/utils/judgeHarnesses/](apps/api/src/utils/judgeHarnesses/) |
| Execution routing (balanced providers, lanes, admission) | [apps/api/src/utils/executionRouting.ts](apps/api/src/utils/executionRouting.ts) |
| NAT-safe rate-limit keys | [apps/api/src/utils/rateLimitKey.ts](apps/api/src/utils/rateLimitKey.ts) |
| NAT-safe socket connect keying | [apps/api/src/utils/socketConnectKey.ts](apps/api/src/utils/socketConnectKey.ts) |
| Judge stress harness (sim upstreams + real worker) | [scripts/stress/run-stress.mjs](scripts/stress/run-stress.mjs) |
...
| Backdate policy (pure, unit-tested) | [apps/api/src/utils/backdate.ts](apps/api/src/utils/backdate.ts) |
| Retro-records router (PRES/SA) | [apps/api/src/routes/backdate.ts](apps/api/src/routes/backdate.ts) |
| Backdate console UI (unlisted) | [apps/web/src/pages/admin/AdminBackdate.tsx](apps/web/src/pages/admin/AdminBackdate.tsx) |
| Shared certificate backdate control | [apps/web/src/components/certificates/CertificateBackdateControl.tsx](apps/web/src/components/certificates/CertificateBackdateControl.tsx) |
| Contest round+registration read cache | [apps/api/src/competition/roundCache.ts](apps/api/src/competition/roundCache.ts) |
...
| QOTD today/summary read cache | [apps/api/src/utils/qotdTodayCache.ts](apps/api/src/utils/qotdTodayCache.ts) |
...
| Batched certificate view counter | [apps/api/src/utils/certificateViewCounter.ts](apps/api/src/utils/certificateViewCounter.ts) |
