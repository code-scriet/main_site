...
- [NotifMenu.tsx](apps/web/src/components/dashboard/NotifMenu.tsx) — bell popover, All/Unread tabs, refetch every 30s when open, "Mark all as read". Clicking a **broadcast** item with a body opens [NotificationDialog.tsx](apps/web/src/components/dashboard/NotificationDialog.tsx) (full message rendered as Markdown with clickable links + an "Open …" action for its link) instead of navigating away; action items (invitations/certs/quiz) still navigate directly. The dialog stamps `data-dashboard`/`data-accent` on its portaled content so the `--ds-*`/`--accent` tokens resolve.
