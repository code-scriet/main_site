...
- **Contest relay + plagiarism offload (Phase H):** because this box is mostly idle, it also hosts the contest `/competition` Socket.io namespace (auth via shared JWT; rooms gated by `pg` registration / admin role) and three `INTERNAL_API_SECRET`-gated internal endpoints the main API calls server-to-server: `POST /internal/contest-emit` (dumb relay → fan-out to rooms), `POST /internal/disconnect-user` (force-logout), `POST /internal/plagiarism` (runs the O(N²) similarity from `plagiarism.js`, the JS mirror of [apps/api/src/competition/plagiarism.ts](apps/api/src/competition/plagiarism.ts)). No scoring/judging here — only connections + the plagiarism batch. Deps: `socket.io`.
...
| `INTERNAL_API_SECRET` | no | shared secret for main API ↔ playground internal calls (contest relay emit/disconnect + plagiarism offload). Unset ⇒ no relay/offload (clients poll, plagiarism runs inline). Set on **both** services. |
| `PLAYGROUND_API_URL` | no | main API → playground execute-server origin (the `/competition` relay + `/internal/plagiarism`). Unset ⇒ inline + polling. |
