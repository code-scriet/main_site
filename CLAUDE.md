...
- **Purpose allowlist (token partitioning):** access tokens never carry a `purpose` claim; every special-purpose token signed with the shared secret does (`attendance`, `oauth_exchange`, `invitation_claim`, `quiz_access`). Both auth middlewares, `verifyToken` (which also covers socket auth), and the playground's `optionalAuth` reject any purpose-carrying JWT for session auth. New special-purpose tokens MUST set a `purpose` claim.
