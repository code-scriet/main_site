...
- **Proctor lockdown (Phase G):** a proctored round additionally enforces **fullscreen** (exit = instant violation→lock) + **copy-paste block** (paste = instant violation→lock) via `useProctor` options; the DSA arena is lock-only (drafts preserved), IMAGE_TARGET still auto-submits-then-locks.
