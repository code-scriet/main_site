...
| Optional native | `better-sqlite3` (S6 quiz crash snapshots) + `eiows` (opt-in C++ WS engine, api + playground) — both `optionalDependencies` with try/require fallbacks: a failed native build degrades to a logged no-op / stock `ws`, never a broken install or boot |
...
| `WS_ENGINE` | no | `eiows` opts the Socket.io servers (main API + playground relay) into the C++ eiows WebSocket engine (µWS fork; same wire protocol, ~10x lower per-connection memory). Optional dependency + try/require fallback: a missing/failed native build logs and falls back to the default `ws` engine, never breaking startup. Unset = stock `ws`. |
