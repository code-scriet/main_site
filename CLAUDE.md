...
- **Judge pipeline:** `problemsCore.ts` → `codeJudge.ts` → per-request provider from [utils/executionRouting.ts](apps/api/src/utils/executionRouting.ts) (`Settings.codeExecutionProvider`: `wandbox` | `godbolt` | **`balanced`** = least-loaded split across both, JS pinned to Wandbox, infra-failing host cooled down 45s) → CF Worker (`workers/executor.js`), **with the other provider as automatic fallback inside the worker** (see Execution Resilience below). Concurrency is **per provider per lane** (submit 5 / testrun 10 each, env-tunable `JUDGE_SUBMIT_CONCURRENCY`/`JUDGE_TESTRUN_CONCURRENCY`), so balanced mode ~doubles judge throughput.
...
### Execution Resilience (balanced dual-provider routing + worker fallback chain)
...
| `CODE_EXECUTION_PROVIDER` | no | fallback (judge + playground) when the `Settings.codeExecutionProvider` column/DB is unavailable (`wandbox`\|`godbolt`\|`balanced`, default `wandbox`). Normal path reads the DB setting. |
| `JUDGE_SUBMIT_CONCURRENCY` | no | max concurrent judge submits **per provider** (default 5). Balanced mode's platform total = 2×. Sized to today's per-upstream pressure — raise only with evidence the upstream tolerates it. |
