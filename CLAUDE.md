...
- **DSA submissions** go to `ProblemSubmission` (contextType=`CONTEST`, contextKey=round.id). Judged by judge. **Contestant entry:** `CompetitionSolvePage`/AdminCompetition/Coding-hub link to `/competition/:roundId/solve/:problemId`, which redirects to the playground with `?contest=<roundId>&problem=<problemId>`; `PlaygroundPage` reads `?contest` and builds a **CONTEST** `QOTDSolverContext` (`contextKey=roundId`, submit gated on round `ACTIVE`) so run/submit go through `/api/problems/:id/{run,submit}` in the contest context. **Without the `?contest` handler the solve silently falls back to PRACTICE** (the submit never reaches the round) — keep it wired. The unused `submitCompetitionCode` DSA branch on `/api/competition/:roundId/submit` is legacy; the live path is the Problems judge.
...
- **Finish + ranks:** IMAGE_TARGET requires scored submissions in judging; DSA can finish directly from LOCKED/JUDGING. Ranks are auto-computed from scores (standard 1224) by `recomputeRoundRanks()` — invoked on finish **and** when `PATCH /score/:submissionId` corrects a score on an already-FINISHED round (so stored `rank` never drifts out of sync with `score`). An explicit `rank` in the score payload is honored and skips the recompute (manual-override escape hatch).
