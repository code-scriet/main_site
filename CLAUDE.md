...
### Problems endpoints
List/admin-all/admin-reset-cap/admin-pending-cap-requests · create/update/delete/publish · run/submit/my-submission/leaderboard/all-submissions · override/:submissionId · rejudge + rejudge-status/:jobId · request-cap · `:id/appeal` (User — flag a non-accepted submission for review) · `admin/review-queue` (Admin — `needsReview` submissions for manual grading) · `admin/reopen/:submissionId/{accept,reject}` (Admin — accept/reject a held reopened-QOTD solve; accept flips PENDING→ACCEPTED + recomputes streak/leaderboard). Per-problem `/:id/leaderboard` excludes `verdict=PENDING` (held reopen solves never rank).

### QOTD endpoints
`/today` · `/history` · `/leaderboard/total` · `/stats/leaderboard` · `/:qotdId/leaderboard` · `/:id` GET/PUT/DELETE · POST create (CORE+) · `/publish`, `/hold`, `/publish-practice`, `/unpublish-practice` (Admin) · `/reopen`, `/close-reopen` (PRES/SA — reopen a past QOTD via private signed link) · `/submit` (legacy text) · `/leaderboard/around-me` (User, Dashboard v2; `RANK() OVER ()` CTE → `{slice, myRank, totalRanked, nextUpDelta, nextUp}`).
