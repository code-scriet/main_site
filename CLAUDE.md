...
- **Proctor lockdown (Phase G):** a proctored round additionally enforces **fullscreen** (exit) + **copy-paste block** (paste) via `useProctor` options. These two "instant" kinds get a per-participant **budget** server-side ([competition.ts](apps/api/src/routes/competition.ts) `INSTANT_VIOLATION_BUDGET`, default 2): the first two are counted + warned (the arena flashes a toast; `POST /proctor/violation` returns `{warning, remaining}`), the **3rd locks** — so a reflexive Ctrl-V / OS-driven fullscreen exit doesn't instant-lock and swamp invigilator unlock throughput. **Tab-away still locks on its first trip** (it already passed a 10s on-screen grace). Every violation is logged + counted regardless. The DSA arena is lock-only (drafts preserved), IMAGE_TARGET still auto-submits-then-locks. Admin lock/unlock pushes ride `contest:proctor` → the arena's `applyProctorPush` applies them instantly (heartbeat poll is the ≤15s fallback).
