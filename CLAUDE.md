...
| `PLAYGROUND_API_URL` | no | main API → playground execute-server origin (the `/competition` relay + `/internal/plagiarism`). The **one** value to set (on the main `codescriet-api` service) to turn the relay/offload on; a bare host is accepted (`https://` assumed). Unset ⇒ inline + polling. |
| `VITE_PLAYGROUND_API_URL` | no | web build-time: execute-server origin the admin monitor opens its `/competition` socket against. Unset in prod ⇒ monitor uses REST polling. (Playground arena uses its own `VITE_API_URL`.) |
