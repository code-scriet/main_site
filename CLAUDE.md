...
| `/api/problems/sheets/*` | Mixed (gated by `problemsEnabled`) | S-09 topic ladders. Mounted **before** `/api/problems` so "sheets" dodges the `/:idOrSlug` catch-all. `GET /` + `GET /:slug` (live per-user progress from PRACTICE-context AC); `POST` create=CORE_MEMBER+ (non-admins forced unpublished); `PUT`/`DELETE`=Admin |
...
Sheet: `id, slug(unique), title, description?, isPublished(default false), createdBy(FK User Restrict), createdAt/updatedAt` · index `[isPublished,createdAt desc]`. Item: `id, sheetId(FK Cascade), problemId(FK Cascade), order(default 0)` · unique `[sheetId,problemId]` · index `[sheetId,order]`. Per-member progress computed live from `ProblemSubmission` (PRACTICE-context ACCEPTED) — no per-user state stored.
...
### QOTD endpoints
`/today` · `/history` (supports `?date=YYYY-MM-DD` single-day lookup + `?includeUnpublished` for staff) · `/history/summary` (optional auth → `{totalPublished, solved, left}` for the member "Full history" header; reuses `getSubmittedQotdIds`) · `/leaderboard/total` · `/stats/leaderboard` · `/:qotdId/leaderboard` · `/:id` GET/PUT/DELETE · POST create (CORE+) · `/publish`, `/hold`, `/publish-practice`, `/unpublish-practice` (Admin) · `/reopen`, `/close-reopen` (PRES/SA — reopen a past QOTD via private signed link) · `/submit` (legacy text) · `/leaderboard/around-me` (User, Dashboard v2; `RANK() OVER ()` CTE → `{slice, myRank, totalRanked, nextUpDelta, nextUp}`).
