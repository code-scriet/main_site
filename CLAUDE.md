...
| Real-time | Socket.io: `/quiz`, `/attendance`, `/notifications`, `/competition` namespaces |
...
2. HTTP + Socket.io (`/quiz` + `/attendance` + `/notifications` + `/competition`).
...
- **Realtime (Phase G, `/competition` Socket.io namespace + bounded in-memory store):** push so no end reloads. [apps/api/src/competition/competitionSocket.ts](apps/api/src/competition/competitionSocket.ts) (contestants join `round:<id>` + a per-user room; admins join `round:<id>:admin`) + [competitionRealtime.ts](apps/api/src/competition/competitionRealtime.ts) (bounded per-active-round store: first-solve map + leaderboard throttle handle, evicted on finish — bounded by active rounds, not users). Events: `contest:leaderboard` (throttled 1/sec, freeze-aware), `:submission` (admin feed), `:firstSolve` (balloons), `:clarification`, `:status` (lobby → synced start / lock / finish), `:proctor` (targeted lock/unlock), `:violation`, `:participant`. Clients: arena `useContestSocket` ([apps/playground](apps/playground/src/hooks/useContestSocket.ts)) updates query caches in place + balloon toasts + refetch on status; admin monitor `useContestAdminSocket` ([apps/web](apps/web/src/hooks/useContestSocket.ts)) debounced-refetches. Polling (Phase E) stays as fallback. `buildDsaLeaderboard` is shared from [contestScoring.ts](apps/api/src/utils/contestScoring.ts).
